/**
 * This file was auto-generated by openapi-typescript.
 * Do not make direct changes to the file.
 */

export interface paths {
    "/api/status": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Status
         * @description Get current system status as JSON.
         */
        get: operations["api_status_api_status_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/config": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Config
         * @description Get current configuration as JSON.
         */
        get: operations["api_config_api_config_get"];
        /**
         * Api Update Config
         * @description Update in-memory configuration (validation only, not saved).
         */
        put: operations["api_update_config_api_config_put"];
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/connections": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Connections
         * @description Test all external connections.
         */
        get: operations["api_connections_api_connections_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/update": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Trigger Update
         * @description Trigger immediate photo update.
         */
        post: operations["api_trigger_update_api_update_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/clear": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Clear Display
         * @description Clear the display.
         */
        post: operations["api_clear_display_api_clear_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/config/save": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Save Config
         * @description Save current in-memory configuration to YAML file.
         */
        post: operations["api_save_config_api_config_save_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/config/revert": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Revert Config
         * @description Revert in-memory config to what's on disk.
         */
        post: operations["api_revert_config_api_config_revert_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/restart": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Restart
         * @description Restart the application.
         */
        post: operations["api_restart_api_restart_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/scheduler/status": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Scheduler Status
         * @description Get scheduler status.
         */
        get: operations["api_scheduler_status_api_scheduler_status_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/scheduler/pause": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Scheduler Pause
         * @description Pause automatic updates (daily e-ink refresh still occurs).
         */
        post: operations["api_scheduler_pause_api_scheduler_pause_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/scheduler/resume": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Scheduler Resume
         * @description Resume automatic updates.
         */
        post: operations["api_scheduler_resume_api_scheduler_resume_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/source/stats": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Source Stats
         * @description Get source statistics.
         *
         *     DEPRECATED: Sources are now managed as plugins.
         *     Use /api/plugins/instances for plugin instance information.
         */
        get: operations["api_source_stats_api_source_stats_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/display/current": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Display Current
         * @description Get current display information.
         */
        get: operations["api_display_current_api_display_current_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/display/preview": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Display Preview
         * @description Serve the current display preview image (if available).
         */
        get: operations["api_display_preview_api_display_preview_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/display/history": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Display History
         * @description Get display history.
         */
        get: operations["api_display_history_api_display_history_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/display/health": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Display Health
         * @description Get e-ink display health metrics.
         */
        get: operations["api_display_health_api_display_health_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/playlists": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Playlists List
         * @description Get list of all playlists.
         */
        get: operations["api_playlists_list_api_playlists_get"];
        put?: never;
        /**
         * Api Playlist Create
         * @description Create a new playlist.
         */
        post: operations["api_playlist_create_api_playlists_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/playlists/{playlist_id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Playlist Details
         * @description Get details for a specific playlist.
         */
        get: operations["api_playlist_details_api_playlists__playlist_id__get"];
        /**
         * Api Playlist Update
         * @description Update a playlist.
         */
        put: operations["api_playlist_update_api_playlists__playlist_id__put"];
        post?: never;
        /**
         * Api Playlist Delete
         * @description Delete a playlist.
         */
        delete: operations["api_playlist_delete_api_playlists__playlist_id__delete"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/playlists/{playlist_id}/activate": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Playlist Activate
         * @description Set a playlist as active.
         */
        post: operations["api_playlist_activate_api_playlists__playlist_id__activate_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/playlists/deactivate": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Playlist Deactivate
         * @description Deactivate the current playlist.
         */
        post: operations["api_playlist_deactivate_api_playlists_deactivate_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/plugins": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Plugins List
         * @description Get list of all available plugins.
         */
        get: operations["api_plugins_list_api_plugins_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/plugins/{plugin_id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Plugin Details
         * @description Get details for a specific plugin.
         */
        get: operations["api_plugin_details_api_plugins__plugin_id__get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/plugins/instances": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Instances List
         * @description Get list of all plugin instances.
         */
        get: operations["api_instances_list_api_plugins_instances_get"];
        put?: never;
        /**
         * Api Instances Create
         * @description Create a new plugin instance.
         */
        post: operations["api_instances_create_api_plugins_instances_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/plugins/instances/{instance_id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Instance Details
         * @description Get details for a specific instance.
         */
        get: operations["api_instance_details_api_plugins_instances__instance_id__get"];
        /**
         * Api Instance Update
         * @description Update an instance.
         */
        put: operations["api_instance_update_api_plugins_instances__instance_id__put"];
        post?: never;
        /**
         * Api Instance Delete
         * @description Delete an instance.
         */
        delete: operations["api_instance_delete_api_plugins_instances__instance_id__delete"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/plugins/instances/{instance_id}/enable": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Instance Enable
         * @description Enable an instance.
         */
        post: operations["api_instance_enable_api_plugins_instances__instance_id__enable_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/plugins/instances/{instance_id}/disable": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Instance Disable
         * @description Disable an instance.
         */
        post: operations["api_instance_disable_api_plugins_instances__instance_id__disable_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/plugins/instances/{instance_id}/test": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Instance Test
         * @description Test run a plugin instance.
         */
        post: operations["api_instance_test_api_plugins_instances__instance_id__test_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/schedules": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Schedules List
         * @description Get all schedule slots.
         */
        get: operations["api_schedules_list_api_schedules_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/schedules/slot": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Schedule Set Slot
         * @description Set a single time slot.
         */
        post: operations["api_schedule_set_slot_api_schedules_slot_post"];
        /**
         * Api Schedule Clear Slot
         * @description Clear a single time slot.
         */
        delete: operations["api_schedule_clear_slot_api_schedules_slot_delete"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/schedules/slots/bulk": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Schedule Bulk Set
         * @description Set multiple slots at once.
         */
        post: operations["api_schedule_bulk_set_api_schedules_slots_bulk_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/schedules/current": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api Schedule Current
         * @description Get what's currently scheduled for right now.
         */
        get: operations["api_schedule_current_api_schedules_current_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/schedules/clear": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * Api Schedule Clear All
         * @description Clear all schedule slots.
         */
        post: operations["api_schedule_clear_all_api_schedules_clear_post"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/system/info": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api System Info
         * @description Get system information.
         */
        get: operations["api_system_info_api_system_info_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/system/logs": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api System Logs
         * @description Get system logs.
         */
        get: operations["api_system_logs_api_system_logs_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/system/logs/export": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Api System Logs Export
         * @description Export system logs as text file.
         */
        get: operations["api_system_logs_export_api_system_logs_export_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Index
         * @description Serve SPA for root route.
         */
        get: operations["index__get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/plugins": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Plugins Page
         * @description Serve SPA for plugins page.
         */
        get: operations["plugins_page_plugins_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/schedule": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Schedule Page
         * @description Serve SPA for schedule page.
         */
        get: operations["schedule_page_schedule_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/system": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * System Page
         * @description Serve SPA for system page.
         */
        get: operations["system_page_system_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/config": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Config Page
         * @description Serve SPA for config page.
         */
        get: operations["config_page_config_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/playlists": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Playlists Page
         * @description Serve SPA for playlists page.
         */
        get: operations["playlists_page_playlists_get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/assets/{filename}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * Serve Assets
         * @description Serve built assets from dist directory.
         */
        get: operations["serve_assets_assets__filename__get"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
}
export type webhooks = Record<string, never>;
export interface components {
    schemas: {
        /**
         * APIResponse
         * @description Base API response wrapper.
         */
        APIResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
        };
        /**
         * APIResponseWithData
         * @description API response with generic data field.
         */
        APIResponseWithData: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            /** Data */
            data?: unknown | null;
        };
        /**
         * BulkSlotSetRequest
         * @description Request body for bulk setting slots.
         */
        BulkSlotSetRequest: {
            /** Slots */
            slots: components["schemas"]["SlotSetRequest"][];
        };
        /**
         * DisplayCurrentData
         * @description Current display information.
         */
        DisplayCurrentData: {
            /** Image Id */
            image_id?: string | null;
            /** Last Update */
            last_update?: string | null;
            /**
             * Plugin Name
             * @default Unknown
             */
            plugin_name: string;
            /**
             * Instance Name
             * @default Unknown
             */
            instance_name: string;
            /** Status */
            status: string;
            /** Has Preview */
            has_preview: boolean;
            /**
             * Display Count
             * @default 0
             */
            display_count: number;
        };
        /**
         * DisplayCurrentResponse
         * @description Response for /api/display/current endpoint.
         */
        DisplayCurrentResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            data?: components["schemas"]["DisplayCurrentData"] | null;
        };
        /**
         * DisplayHealthData
         * @description Display health metrics.
         */
        DisplayHealthData: {
            /**
             * Refresh Count
             * @default 0
             */
            refresh_count: number;
            /** Last Refresh */
            last_refresh?: string | null;
            /** Status */
            status: string;
            /**
             * Error Count
             * @default 0
             */
            error_count: number;
        };
        /**
         * DisplayHealthResponse
         * @description Response for /api/display/health endpoint.
         */
        DisplayHealthResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            data?: components["schemas"]["DisplayHealthData"] | null;
        };
        /** HTTPValidationError */
        HTTPValidationError: {
            /** Detail */
            detail?: components["schemas"]["ValidationError"][];
        };
        /**
         * InstanceCreateRequest
         * @description Request body for creating a plugin instance.
         */
        InstanceCreateRequest: {
            /** Plugin Id */
            plugin_id: string;
            /** Name */
            name: string;
            /** Settings */
            settings?: {
                [key: string]: unknown;
            };
        };
        /**
         * InstanceData
         * @description Plugin instance data.
         */
        InstanceData: {
            /** Id */
            id: string;
            /** Plugin Id */
            plugin_id: string;
            /** Name */
            name: string;
            /** Settings */
            settings?: {
                [key: string]: unknown;
            };
            /** Enabled */
            enabled: boolean;
            /** Created At */
            created_at: string;
            /** Updated At */
            updated_at: string;
        };
        /**
         * InstanceResponse
         * @description Response for /api/plugins/instances/<id> endpoint.
         */
        InstanceResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            data?: components["schemas"]["InstanceData"] | null;
        };
        /**
         * InstanceUpdateRequest
         * @description Request body for updating a plugin instance.
         */
        InstanceUpdateRequest: {
            /** Name */
            name?: string | null;
            /** Settings */
            settings?: {
                [key: string]: unknown;
            } | null;
        };
        /**
         * InstancesListResponse
         * @description Response for /api/plugins/instances endpoint.
         */
        InstancesListResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            /** Data */
            data?: components["schemas"]["InstanceData"][] | null;
        };
        /**
         * LogEntry
         * @description Log entry data.
         */
        LogEntry: {
            /** Timestamp */
            timestamp: string;
            /** Level */
            level: string;
            /** Message */
            message: string;
        };
        /**
         * PlaylistCreateRequest
         * @description Request body for creating a playlist.
         */
        PlaylistCreateRequest: {
            /** Name */
            name: string;
            /**
             * Description
             * @default
             */
            description: string;
            /**
             * Playback Mode
             * @default sequential
             */
            playback_mode: string;
            /** Items */
            items?: components["schemas"]["PlaylistItemData"][];
        };
        /**
         * PlaylistData
         * @description Playlist data.
         */
        PlaylistData: {
            /** Id */
            id: string;
            /** Name */
            name: string;
            /**
             * Description
             * @default
             */
            description: string;
            /** Enabled */
            enabled: boolean;
            /**
             * Playback Mode
             * @default sequential
             */
            playback_mode: string;
            /** Items */
            items?: components["schemas"]["PlaylistItemData"][];
            /** Created At */
            created_at: string;
            /** Updated At */
            updated_at: string;
        };
        /**
         * PlaylistItemData
         * @description Playlist item data.
         */
        PlaylistItemData: {
            /** Instance Id */
            instance_id: string;
            /** Duration Seconds */
            duration_seconds: number;
            /**
             * Order
             * @default 0
             */
            order: number;
            /** Conditions */
            conditions?: {
                [key: string]: unknown;
            } | null;
            /**
             * Weight
             * @default 1
             */
            weight: number;
        };
        /**
         * PlaylistResponse
         * @description Response for /api/playlists/<id> endpoint.
         */
        PlaylistResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            data?: components["schemas"]["PlaylistData"] | null;
        };
        /**
         * PlaylistUpdateRequest
         * @description Request body for updating a playlist.
         */
        PlaylistUpdateRequest: {
            /** Name */
            name?: string | null;
            /** Description */
            description?: string | null;
            /** Enabled */
            enabled?: boolean | null;
            /** Playback Mode */
            playback_mode?: string | null;
            /** Items */
            items?: components["schemas"]["PlaylistItemData"][] | null;
        };
        /**
         * PlaylistsListResponse
         * @description Response for /api/playlists endpoint.
         */
        PlaylistsListResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            /** Data */
            data?: components["schemas"]["PlaylistData"][] | null;
            /** Active Playlist Id */
            active_playlist_id?: string | null;
        };
        /**
         * PluginData
         * @description Plugin metadata.
         */
        PluginData: {
            /** Id */
            id: string;
            /** Display Name */
            display_name: string;
            /** Class Name */
            class_name: string;
            /** Description */
            description: string;
            /** Author */
            author: string;
            /** Version */
            version: string;
            /** Icon */
            icon?: string | null;
            /** Settings Schema */
            settings_schema?: {
                [key: string]: components["schemas"]["SettingsField"];
            };
        };
        /**
         * PluginResponse
         * @description Response for /api/plugins/<id> endpoint.
         */
        PluginResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            data?: components["schemas"]["PluginData"] | null;
        };
        /**
         * PluginsListResponse
         * @description Response for /api/plugins endpoint.
         */
        PluginsListResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            /** Data */
            data?: components["schemas"]["PluginData"][] | null;
        };
        /**
         * ScheduleCurrentData
         * @description Current schedule information.
         */
        ScheduleCurrentData: {
            /** Has Content */
            has_content: boolean;
            /**
             * Source Type
             * @default none
             */
            source_type: string;
            /** Target Type */
            target_type?: string | null;
            /** Target Id */
            target_id?: string | null;
            /** Target Name */
            target_name?: string | null;
            /** Instance */
            instance?: {
                [key: string]: unknown;
            } | null;
            /** Day */
            day?: number | null;
            /** Hour */
            hour?: number | null;
        };
        /**
         * ScheduleCurrentResponse
         * @description Response for /api/schedules/current endpoint.
         */
        ScheduleCurrentResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            data?: components["schemas"]["ScheduleCurrentData"] | null;
        };
        /**
         * ScheduleSlotsResponse
         * @description Response for /api/schedules endpoint.
         */
        ScheduleSlotsResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            /** Slots */
            slots?: {
                [key: string]: {
                    [key: string]: unknown;
                };
            };
            /**
             * Slot Count
             * @default 0
             */
            slot_count: number;
        };
        /**
         * SchedulerStatus
         * @description Scheduler status information.
         */
        SchedulerStatus: {
            /** Paused */
            paused: boolean;
            /** Next Update */
            next_update?: string | null;
            /** Last Update */
            last_update?: string | null;
        };
        /**
         * SchedulerStatusResponse
         * @description Response for /api/scheduler/status endpoint.
         */
        SchedulerStatusResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            data?: components["schemas"]["SchedulerStatus"] | null;
            status?: components["schemas"]["SchedulerStatus"] | null;
        };
        /**
         * SettingsField
         * @description Schema for a single settings field.
         */
        SettingsField: {
            /** Type */
            type: string;
            /** Label */
            label: string;
            /** Description */
            description?: string | null;
            /**
             * Required
             * @default false
             */
            required: boolean;
            /** Default */
            default?: unknown | null;
            /** Options */
            options?: {
                [key: string]: string;
            }[] | null;
            /** Min */
            min?: number | null;
            /** Max */
            max?: number | null;
            /** Placeholder */
            placeholder?: string | null;
        };
        /**
         * SlotClearRequest
         * @description Request body for clearing a slot.
         */
        SlotClearRequest: {
            /** Day */
            day?: number | null;
            /** Hour */
            hour?: number | null;
        };
        /**
         * SlotSetRequest
         * @description Request body for setting a schedule slot.
         */
        SlotSetRequest: {
            /** Day */
            day: number;
            /** Hour */
            hour: number;
            /**
             * Target Type
             * @default instance
             */
            target_type: string;
            /** Target Id */
            target_id: string;
        };
        /**
         * SlotSetResponse
         * @description Response for setting a slot.
         */
        SlotSetResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            /** Slot */
            slot?: {
                [key: string]: unknown;
            } | null;
        };
        /**
         * SystemInfoData
         * @description System information data.
         */
        SystemInfoData: {
            /** Cpu Percent */
            cpu_percent: number;
            /** Memory Percent */
            memory_percent: number;
            /** Disk Percent */
            disk_percent: number;
            /** Temperature */
            temperature?: number | null;
            /** Uptime */
            uptime: string;
            /** Platform */
            platform: string;
        };
        /**
         * SystemInfoResponse
         * @description Response for /api/system/info endpoint.
         */
        SystemInfoResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            data?: components["schemas"]["SystemInfoData"] | null;
        };
        /**
         * SystemLogsResponse
         * @description Response for /api/system/logs endpoint.
         */
        SystemLogsResponse: {
            /** Success */
            success: boolean;
            /** Message */
            message?: string | null;
            /** Error */
            error?: string | null;
            /** Data */
            data?: components["schemas"]["LogEntry"][] | null;
        };
        /** ValidationError */
        ValidationError: {
            /** Location */
            loc: (string | number)[];
            /** Message */
            msg: string;
            /** Error Type */
            type: string;
        };
    };
    responses: never;
    parameters: never;
    requestBodies: never;
    headers: never;
    pathItems: never;
}
export type $defs = Record<string, never>;
export interface operations {
    api_status_api_status_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponseWithData"];
                };
            };
        };
    };
    api_config_api_config_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponseWithData"];
                };
            };
        };
    };
    api_update_config_api_config_put: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": {
                    [key: string]: unknown;
                };
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_connections_api_connections_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponseWithData"];
                };
            };
        };
    };
    api_trigger_update_api_update_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
        };
    };
    api_clear_display_api_clear_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
        };
    };
    api_save_config_api_config_save_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponseWithData"];
                };
            };
        };
    };
    api_revert_config_api_config_revert_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
        };
    };
    api_restart_api_restart_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
        };
    };
    api_scheduler_status_api_scheduler_status_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["SchedulerStatusResponse"];
                };
            };
        };
    };
    api_scheduler_pause_api_scheduler_pause_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["SchedulerStatusResponse"];
                };
            };
        };
    };
    api_scheduler_resume_api_scheduler_resume_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["SchedulerStatusResponse"];
                };
            };
        };
    };
    api_source_stats_api_source_stats_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponseWithData"];
                };
            };
        };
    };
    api_display_current_api_display_current_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["DisplayCurrentResponse"];
                };
            };
        };
    };
    api_display_preview_api_display_preview_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": unknown;
                };
            };
        };
    };
    api_display_history_api_display_history_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponseWithData"];
                };
            };
        };
    };
    api_display_health_api_display_health_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["DisplayHealthResponse"];
                };
            };
        };
    };
    api_playlists_list_api_playlists_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["PlaylistsListResponse"];
                };
            };
        };
    };
    api_playlist_create_api_playlists_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["PlaylistCreateRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["PlaylistResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_playlist_details_api_playlists__playlist_id__get: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                playlist_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["PlaylistResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_playlist_update_api_playlists__playlist_id__put: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                playlist_id: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["PlaylistUpdateRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["PlaylistResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_playlist_delete_api_playlists__playlist_id__delete: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                playlist_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_playlist_activate_api_playlists__playlist_id__activate_post: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                playlist_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_playlist_deactivate_api_playlists_deactivate_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
        };
    };
    api_plugins_list_api_plugins_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["PluginsListResponse"];
                };
            };
        };
    };
    api_plugin_details_api_plugins__plugin_id__get: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                plugin_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["PluginResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_instances_list_api_plugins_instances_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["InstancesListResponse"];
                };
            };
        };
    };
    api_instances_create_api_plugins_instances_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["InstanceCreateRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["InstanceResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_instance_details_api_plugins_instances__instance_id__get: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                instance_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["InstanceResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_instance_update_api_plugins_instances__instance_id__put: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                instance_id: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["InstanceUpdateRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["InstanceResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_instance_delete_api_plugins_instances__instance_id__delete: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                instance_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_instance_enable_api_plugins_instances__instance_id__enable_post: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                instance_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_instance_disable_api_plugins_instances__instance_id__disable_post: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                instance_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_instance_test_api_plugins_instances__instance_id__test_post: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                instance_id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_schedules_list_api_schedules_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ScheduleSlotsResponse"];
                };
            };
        };
    };
    api_schedule_set_slot_api_schedules_slot_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["SlotSetRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["SlotSetResponse"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_schedule_clear_slot_api_schedules_slot_delete: {
        parameters: {
            query?: {
                day?: number | null;
                hour?: number | null;
            };
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: {
            content: {
                "application/json": components["schemas"]["SlotClearRequest"] | null;
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponseWithData"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_schedule_bulk_set_api_schedules_slots_bulk_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["BulkSlotSetRequest"];
            };
        };
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponseWithData"];
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
    api_schedule_current_api_schedules_current_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ScheduleCurrentResponse"];
                };
            };
        };
    };
    api_schedule_clear_all_api_schedules_clear_post: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["APIResponseWithData"];
                };
            };
        };
    };
    api_system_info_api_system_info_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["SystemInfoResponse"];
                };
            };
        };
    };
    api_system_logs_api_system_logs_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["SystemLogsResponse"];
                };
            };
        };
    };
    api_system_logs_export_api_system_logs_export_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": unknown;
                };
            };
        };
    };
    index__get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": unknown;
                };
            };
        };
    };
    plugins_page_plugins_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": unknown;
                };
            };
        };
    };
    schedule_page_schedule_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": unknown;
                };
            };
        };
    };
    system_page_system_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": unknown;
                };
            };
        };
    };
    config_page_config_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": unknown;
                };
            };
        };
    };
    playlists_page_playlists_get: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": unknown;
                };
            };
        };
    };
    serve_assets_assets__filename__get: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                filename: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Successful Response */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": unknown;
                };
            };
            /** @description Validation Error */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["HTTPValidationError"];
                };
            };
        };
    };
}
